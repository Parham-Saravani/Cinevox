const Card = (title, slug, type, genres, poster, isSlider) => {
  return `
    <div class="group  h-60! max-sm:h-48! ${isSlider ? "swiper-slide" : ""} ${isSlider ? "w-45! max-xl:w-40! max-md:w-38! max-sm:w-32!" : "w-full"}">
        <a href="${type === "movie" ? `/movie.html?title=${slug}` : `/serie.html?title=${slug}`}" class="relative h-full">
            <img class="animate-fadeIn object-cover h-full w-full rounded-xl card-image bg-gray-900 animate-pulse" src="public${poster}" alt="" loading="lazy">
